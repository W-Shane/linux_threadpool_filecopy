 ...
 * 绑定拼音输入法到指定文本框
 * @param obj      拼音输入法对象
 * @param textarea 要绑定的文本框对象
 */
void lv_100ask_pinyin_ime_attach(lv_obj_t * obj, lv_obj_t * textarea)
{
    LV_ASSERT_OBJ(obj, MY_CLASS);       // 校验输入法对象合法性
    LV_ASSERT_OBJ(textarea, &lv_textarea_class);  // 校验文本框对象合法性

    lv_100ask_pinyin_ime_t * pinyin_ime = (lv_100ask_pinyin_ime_t *)obj;
    // 将输入法的键盘绑定到文本框（核心逻辑）
    lv_keyboard_set_textarea(pinyin_ime->kb, textarea);
}
